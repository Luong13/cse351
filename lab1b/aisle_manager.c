...
#define SECTION_MASK 0x000000000000FF
...
unsigned short get_section(unsigned long* aisle, int index) {
  ...
  unsigned short* ptr = (unsigned short*)aisle;
  ptr += index;
  return *ptr;
}
...
void set_spaces(unsigned long* aisle, int index, unsigned short new_spaces) {
  // TODO: implement this method
  if ((new_spaces & ID_MASK) == 0) {
	  unsigned short* ptr = (unsigned short*)aisle;
	  ptr += index;
  	  *ptr = (*ptr & ID_MASK) | new_spaces;
...
void set_id(unsigned long* aisle, int index, unsigned short new_id) {
  ...
  if ((new_id & 0xFFC0) == 0) {
	  unsigned short* ptr = (unsigned short*)aisle;
	  ptr += index;
	  *ptr  = (*ptr & SPACES_MASK) | (new_id<<NUM_SPACES); 
  }
}
...
unsigned short num_items(unsigned long* aisle, int index) {
  // TODO: implement this method
  unsigned short* ptr = (unsigned short*)aisle + index;
  unsigned short items = *ptr & SPACES_MASK;
  int count = 0;
...
void add_items(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
  unsigned short* ptr = (unsigned short*)aisle + index;
  unsigned short items = *ptr;
  unsigned short item_mask = 1;
  for (int i = 0; i < NUM_SPACES; i++) {
	  if (n > 0 && (items & item_mask) == 0) {
		  items = items | item_mask;
		  n--;
	  }
	  item_mask<<=1;
  }
...
void rotate_items_left(unsigned long* aisle, int index, int n) {
  // TODO: implement this method
  while (n > (NUM_SPACES - 1)) {
	  n = n - NUM_SPACES;
  }
  unsigned short* ptr = (unsigned short*)aisle + index;
  unsigned int items = *ptr & SPACES_MASK;
  items<<=n;
  unsigned int looped_items = (items & 0xFFFFFC00)>>NUM_SPACES;
  items = (items & 0x000003FF) | looped_items;
  *ptr = (*ptr & ID_MASK) | items;
}
